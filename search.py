...
def uniformCostSearch(problem: SearchProblem):
    ...
    startState = problem.getStartState()
    priorq.push((startState, [], 0), 0) 

    visited_costs = {}

    while not priorq.isEmpty():
        cur_state, path, cost = priorq.pop()  

        if cur_state in visited_costs and visited_costs[cur_state] <= cost:
            continue

        visited_costs[cur_state] = cost  
        ...
        for successor, action, stepCost in problem.getSuccessors(cur_state):
            ...
            priorq.push((successor, path + [action], new_cost), new_cost)
    return []
...
def aStarSearch(problem: SearchProblem, heuristic=nullHeuristic):
    ...
    while not priorq.isEmpty():
        ...
        for successor, action, stepCost in problem.getSuccessors(cur_state):
            ...
            priorq.push((successor, path + [action], new_cost), f_cost)
    return []
